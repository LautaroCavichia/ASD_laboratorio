...
def plot_results(results, operation, case, scale='linear', different_data_structures=True, export=False):
    ...
    if export:
        plt.savefig(f'output/{operation}_performance_comparison_{case}_case.png')
    plt.show()
...
def export_to_latex_direct(results, operation_case, export = False):
...
\\end{figure}""" % operation_case.replace('_', ' ').capitalize()
    ...
    if export:
        with open(f'output/{operation_case}_table.tex', 'w') as f:
            f.write(latex_code)
